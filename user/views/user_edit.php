<?php
/**
 * 后台视图：用户新增/编辑（layui 表单，下拉经 form 模块渲染）
 */
defined('APP_BOOT') or exit;
$isEdit = $user !== null;
$roles = array(
    'user'   => admin_t('admin.user.role_user'),
    'editor' => admin_t('admin.user.role_editor'),
    'admin'  => admin_t('admin.user.role_admin'),
);
// 安装管理员账号：角色与状态锁定，防止误降权/误禁用
$lockRoot = $isEdit && !empty($isRoot);
?>
<div class="card">
    <form method="post" action="<?php echo e(site_base_admin('user/save')); ?>" class="layui-form v-form">
        <?php echo Csrf::field(); ?>
        <input type="hidden" name="id" value="<?php echo $isEdit ? (int) $user['id'] : 0; ?>">

        <?php if (!$isEdit): ?>
        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.field_username_new')); ?></label>
            <input type="text" name="username" pattern="[A-Za-z0-9_]{3,32}" required class="layui-input">
        </div>
        <?php else: ?>
        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.col_username')); ?></label>
            <input type="text" value="<?php echo e($user['username']); ?>" disabled class="layui-input">
        </div>
        <?php endif; ?>

        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.col_nickname')); ?></label>
            <input type="text" name="nickname" value="<?php echo $isEdit ? e($user['nickname']) : ''; ?>" class="layui-input">
        </div>
        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.field_email')); ?></label>
            <input type="email" name="email" value="<?php echo $isEdit && $user['email'] !== null ? e($user['email']) : ''; ?>" class="layui-input">
        </div>
        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.field_phone')); ?></label>
            <input type="text" name="phone" pattern="1[3-9][0-9]{9}" value="<?php echo $isEdit ? e($user['phone']) : ''; ?>" class="layui-input">
        </div>
        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.field_role')); ?><?php echo $lockRoot ? e(admin_t('admin.user.root_no_demote')) : ''; ?></label>
            <?php if ($lockRoot): ?>
            <input type="hidden" name="role" value="admin">
            <select disabled>
                <option selected><?php echo e(admin_t('admin.user.role_admin')); ?></option>
            </select>
            <?php else: ?>
            <select name="role">
                <?php foreach ($roles as $roleKey => $roleName): ?>
                <option value="<?php echo e($roleKey); ?>"
                    <?php echo $isEdit && $user['role'] === $roleKey ? 'selected' : ''; ?>><?php echo e($roleName); ?></option>
                <?php endforeach; ?>
            </select>
            <?php endif; ?>
        </div>
        <?php if ($isEdit): ?>
        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.field_status')); ?></label>
            <?php if ($lockRoot): ?>
            <input type="hidden" name="status" value="1">
            <select disabled>
                <option selected><?php echo e(admin_t('admin.user.status_ok')); ?></option>
            </select>
            <?php else: ?>
            <select name="status">
                <option value="1" <?php echo (int) $user['status'] === 1 ? 'selected' : ''; ?>><?php echo e(admin_t('admin.user.status_ok')); ?></option>
                <option value="0" <?php echo (int) $user['status'] !== 1 ? 'selected' : ''; ?>><?php echo e(admin_t('admin.user.opt_disabled')); ?></option>
            </select>
            <?php endif; ?>
        </div>
        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.field_new_password')); ?></label>
            <input type="password" name="new_password" autocomplete="new-password" class="layui-input">
        </div>
        <div class="layui-form-item">
            <label class="v-label"><?php echo e(admin_t('admin.user.field_current_password')); ?></label>
            <input type="password" name="current_password" autocomplete="current-password" class="layui-input">
        </div>
        <?php else: ?>
        <p class="tip"><?php echo e(admin_t('admin.user.create_tip')); ?></p>
        <?php endif; ?>
        <div class="layui-form-item">
            <label style="display:flex;gap:6px;align-items:center">
                <input type="checkbox" name="force_change" value="1">
                <?php echo e(admin_t('admin.user.force_change')); ?>
            </label>
        </div>
        <button class="layui-btn" type="submit"><i class="layui-icon layui-icon-ok"></i> <?php echo e(admin_t('admin.common.save')); ?></button>
        <a class="layui-btn layui-btn-primary" href="<?php echo e(site_base_admin('user/list')); ?>"><?php echo e(admin_t('admin.common.back')); ?></a>
    </form>
</div>
