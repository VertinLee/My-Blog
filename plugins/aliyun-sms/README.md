...
本插件通过阿里云**号码认证服务（Dypnsapi）**发送与核验短信验证码，签名客户端为手写
HMAC-SHA1 RPC 实现（`AliyunRpc.php`，仅依赖 cURL），不依赖官方 SDK 与 Composer。
插件**预装但默认禁用**，在后台「插件管理」中启用并配置后生效。

## 1. 工作方式

验证码由阿里云生成与核验（端点 `dypnsapi.aliyuncs.com`）：模板中验证码位置写 `##code##`，
发送用 `SendSmsVerifyCode`，核验用 `CheckSmsVerifyCode`。本站 `verify_codes` 表仅用于
频控记账，不参与核验。
...
2. 签名与模板：**不支持自定义签名**。官方接口 `SendSmsVerifyCode` 仅接受
   ...
   **赠送模板**（默认 `100001`）下发；无需审核，但签名/模板内容由平台提供。
   方案名 SchemeName 默认填 `Default`（使用默认方案）。
3. 创建 RAM 子用户并授权（最小权限原则）：
   仅授予 `dypns:SendSmsVerifyCode` 与 `dypns:CheckSmsVerifyCode`，
   建议使用自定义策略仅包含上述 Action，资源范围 `*`。
...
| SchemeName | 方案名（≤20 字符，默认 `Default`） |
| CodeLength | 验证码长度 4-8 |
